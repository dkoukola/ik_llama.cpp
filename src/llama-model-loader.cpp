...
bool llama_model_loader::has_anonymous_mapping() const {
    for (const auto & mapping : mappings) {
        // The NUMA PLE path maps only the shard that owns the PLE tensor and
        // leaves the other mapping slots empty.
        if (mapping && mapping->is_anonymous()) {
            return true;
        }
    }
    return false;
...
bool llama_model_loader::should_defer_ple_mmaps() const {
    return defer_ple && (use_mmap || mmap_ple) && !ple_tensor_index.empty();
}

void llama_model_loader::apply_ple_mmap_policy() const {
    for (size_t idx = 0; idx < ple_tensor_index.file_ranges.size(); ++idx) {
        if (ple_tensor_index.file_ranges[idx].empty()) {
            continue;
        }
        GGML_ASSERT(idx < mappings.size() && mappings[idx] != nullptr);
